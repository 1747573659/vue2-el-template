...
export default class Node {
  ...
  setChecked (value, deep, recursion, passValue, isLast) {
    ...
    var currentNode = this.parent.childNodes[this.parent.childNodes.length - 1]
    if (!(this.shouldLoadData() && !this.store.checkDescendants)) {
      let { all, allWithoutDisable } = getChildState(this.childNodes)
      var result = this.parent.childNodes.slice(0, this.parent.childNodes.length - 1).some(function (computer) {
      ...
      if (isLast && this.level === 5 && this.data.code !== 'COMMON_HEADQUARTERS_MANAGEMENT_VIEW') {
        if (Object.prototype.hasOwnProperty.call(currentNode.data, 'disabled') && currentNode.data.disabled !== '') {
          if (value) {
            ...
            if (!result) {
              currentNode.data.disabled = false
            }
            // currentNode.checked = false
          }
        }
      } else if (!isLast && this.data.code === 'COMMON_HEADQUARTERS_MANAGEMENT_VIEW') {
        if (value) {
          if (!result) {
            currentNode.data.disabled = false
          } else {
      ...
      } else if (isLast && this.level === 5 && this.data.code === 'COMMON_HEADQUARTERS_MANAGEMENT_VIEW') {
      ...
      const handleDescendants = () => {
        if (deep) {
          ...
          for (let i = 0, j = childNodes.length; i < j; i++) {
            ...
            const isCheck = child.disabled ? passValue : passValue
