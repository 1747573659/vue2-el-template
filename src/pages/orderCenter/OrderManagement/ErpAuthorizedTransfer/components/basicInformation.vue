<template>
  <section class="p-information-con" style="padding-bottom:60px" v-loading="checkBasicInformLoad">
...
export default {
  ...
  methods: {
      ...
      this.setOrderSave()
        .then(res => {
          if (this.$route.query.status === 'add') {
            ...
            document.querySelector('.e-tag_active span').innerText = 'ERP授权变更单/编辑'
          }
          this.getDetail()
    ...
    async getProductByPage({ query = '', page = 1, rows = 10 } = {}) {
      try {
        const res = await queryProductCode({ info: query, page, rows, registType: Math.pow(0, this.form.oldRegistType), newOrderType: 37 })
