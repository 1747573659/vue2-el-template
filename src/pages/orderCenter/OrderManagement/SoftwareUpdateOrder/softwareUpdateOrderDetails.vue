...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.updateTagView({ tagRoute: this.$route, title: `软件升级订单/${orderStatus.has(orderStatusVal) ? orderStatus.get(orderStatusVal).name : '新增'}` })
    })
  },
  methods: {
    ...
    async getOperateLog() {
      try {
        const res = await channelDevelopLog({ orderId: Number(this.$route.query.id), orderType: 2 })
