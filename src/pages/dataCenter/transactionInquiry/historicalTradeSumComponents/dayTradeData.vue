  ...
  <div>
    <div class="search-box">
      <div class="xdd_tip">
        ...
        >单次查询日期的最长跨度为31天；查询对象为“全部” 或
        代理商过滤时，结果会包括下级代理商的交易数据
      </div>
      <el-form
        :inline="true"
        :model="form"
        ...
        <el-row>
          <el-col :span="21">
            <el-form-item label="日期">
              ...
              >
              <el-button
                @click="setSearchTime('week')"
                size="small"
                type="default"
                >近7天</el-button
              >
              <el-button
                @click="setSearchTime('month')"
                size="small"
                type="default"
                >近30天</el-button
              >
    ...
    <div class="data-box" v-loading="tableLoading">
      ...
      <!-- TABLE -->
      <el-table
        :data="tableData.cashierMockDTOS"
        ref="table"
      ...
      >
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              v-if="[1, 2, ''].includes(form.searchObject)"
      ...
      pickerOptions: {
        ...
        disabledDate: (time) => {
          if (maxTime) {
            return (
              time.getTime() >
                moment().endOf("day").valueOf() - 24 * 3600 * 1000 ||
              time.getTime() > maxTime ||
              time.getTime() < minTime
            );
          }
          return (
            time.getTime() > moment().endOf("day").valueOf() - 24 * 3600 * 1000
          );
        },
      },
    };
