  ...
  <div class="app-container">
    <div class="search-box">
      <div class="xdd_tip">
        ...
        >只支持查询近半年内的交易流水，单次查询日期的最长跨度为31天
      </div>
      <el-form
        :inline="true"
        :model="formData"
        ...
        <el-row>
          <el-col :span="24">
            ...
            <el-form-item label="交易时间">
              ...
              <el-button
                :disabled="isSubtract"
                @click="setSearchTime('subtract')"
                class="pure-btn_space"
                size="small"
                type="default"
                >前一天</el-button
              >
          ...
          <el-col :span="24">
            ...
            <el-form-item label="交易状态" prop="tradingStatusCode">
              <el-select
                ...
                collapse-tags
                multiple
                v-model="formData.tradingStatusCode"
              >
                <el-option
                  :key="item.codes"
                  :label="item.name"
                  :value="item.codes"
                  v-for="item in tradingStatusData"
                ></el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-table-column align="right" label="操作">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="text"
              v-if="
                scope.row.paymentStatus === 3 || scope.row.paymentStatus === 4
              "
              >退款详情</el-button
            >
          </template>
          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                type="text"
                v-if="
                  permissonCheckMenus('TRANSACTION_MERCHANT_EXPORT_DOWNLOAD')
                "
                >下载</el-button
              >
            </el-link>
            <el-button
              @click="deleteExprot(scope.row)"
              size="small"
              style="margin-left: 5px"
              type="text"
              v-if="permissonCheckMenus('TRANSACTION_MERCHANT_EXPORT_DEL')"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  computed: {
    isSubtract() {
      if (
        moment(this.formData.transactionTime[0])
          .startOf("day")
          .subtract(1, "days")
          .valueOf() === moment().startOf("day").subtract(6, "months").valueOf()
      ) {
        return true;
      } else {
        return false;
    ...
    isAdd() {
      if (
        moment(this.formData.transactionTime[1]).startOf("day").valueOf() ===
        moment().startOf("day").valueOf()
      ) {
        return true;
      } else {
  ...
  methods: {
    ...
    handleSearch() {
      if (this.formData.shopId) {
        if (
          this.formData.transactionTime[1] - this.formData.transactionTime[0] >
          60 * 24 * 60 * 1000 * 31
        ) {
        ...
        if (
          (this.formData.startAmount && !this.formData.endAmount) ||
          (!this.formData.startAmount && this.formData.endAmount)
        ) {
    ...
    async getQueryPage() {
      ...
      let paymentStatus = [];
      if (
        this.formData.tradingStatusCode.length > 0 &&
        this.formData.tradingStatusCode.includes("")
      ) {
        paymentStatus = JSON.parse(
          JSON.stringify(
            this.formData.tradingStatusCode.filter((item) => item !== "")
          )
        );
      } else {
        paymentStatus = JSON.parse(
          JSON.stringify(this.formData.tradingStatusCode)
        );
      }
      const data = {
        page: this.pageNo,
        rows: this.pageSize,
        startDate: moment(this.formData.transactionTime[0]).format(
          "YYYY-MM-DD HH:mm:ss"
        ),
        endDate: moment(this.formData.transactionTime[1]).format(
          "YYYY-MM-DD HH:mm:ss"
        ),
        payChannelType: this.formData.tradingChanneCode
          ? this.formData.tradingChanneCode
          : "", // 交易渠道
        orderTypes:
          this.formData.tradingTypeCode !== ""
            ? [this.formData.tradingTypeCode]
            : [], // 交易类型集合
        paymentMethods:
          this.formData.paymentCode !== "" ? [this.formData.paymentCode] : [], // 支付方式集合
        paymentPlugins:
        ...
        paymentStatus:
          paymentStatus.length > 0
            ? paymentStatus.toString().split(",").map(Number)
            : [], // 交易状态集合
