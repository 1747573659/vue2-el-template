  ...
  <div class="">
    ...
    <div class="data-box">
      <el-table v-loading="loading" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="id" label="代理商编号">
        </el-table-column>
        ...
        <el-table-column label="操作" align="right" width="280px">
          <template slot-scope="scope">
            ...
            <el-button size="small" type="text" @click="handleOperate(scope.row.id, scope.row.status)">{{ scope.row.status | fiterOperateStatus }}</el-button>
            <el-popconfirm style="margin-left: 12px" iconColor="#FFA033" title="你确定要重置密码吗？确定后将对应账号的密码更新为888888" placement="top-start" @onConfirm="resetPsw(scope.row.userId)">
              <el-button slot="reference" type="text" size="small">重置密码</el-button>
            </el-popconfirm>
            <el-button style="margin-left: 12px;" size="small" type="text" @click="handleQuota(scope.row.userId)">分配应用配额</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog title="分配应用配额" :visible.sync="dialogVisible" width="740px">
        <div>
          <div class="dialog-content">
            <aside class="dialog-left">
              <div class="dialog-title">应用名称</div>
              <ul class="dialog-list">
                <li class="dialog-item" :class="{'active': item.id === quotaActiveId}" v-for="item in quotaNameOptions" :key="item.id" @click="handleQuotaName(item.id)">{{ item.name }}</li>
              </ul>
            </aside>
            <section class="dialog-right" v-loading="quotaLoading">
              ...
              <div class="dialog-right-content">
                <div class="quota-item" v-for="item in quotaItems" :key="item.payType">
                  <span>
                    <el-checkbox v-model="item.checked" @change="changeCheckbox(item)"></el-checkbox>
                    {{ item.payType | fiterQuotaPayType }}（剩余配额 <span style="color: #FF6010">{{ item.quotaNo }}</span>，建议售价：<span style="color: #FF6010">{{ (item.saleAmount / 100).toFixed(2) }}</span>元）
                  </span>
                  <span>
                    分配
                    <el-input class="content_input" size="small" v-model="item.input" :disabled="!item.checked" onkeyup="this.value = this.value.replace(/^[1-9][0-9]*]/g,'');" @blur="blur(item)">
                    </el-input>
                    个配额
                  </span>
                </div>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" size="small" @click="onQuotaComfirm">确 定</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
